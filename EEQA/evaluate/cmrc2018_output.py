...
def write_predictions(all_examples, all_features, all_results, n_best_size,
                      max_answer_length, do_lower_case,version_2_with_negative=False,
    ...
    for (example_index, example) in enumerate(tqdm(all_examples)):
        features = example_index_to_features[example['qid']]
        prelim_predictions = []
        # keep track of the minimum score of null start+end of position 0
        score_null = 1000000  # large and positive
        ...
        for (feature_index, feature) in enumerate(features):
            ...
            #print(f'start indexs:{start_indexes}')
            #print(f'end indexs:{end_indexes}')
            # if we could have irrelevant answers, get the min score of irrelevant
        ...
        #print(f'prelim_predictions:{prelim_predictions}')
        ...
        for pred in prelim_predictions:
            ...
            if pred.start_index > 0:  # this is a non-null prediction
                ...
                tok_text = "".join(tok_tokens) if is_chinese else " ".join(tok_tokens)
                ...
                orig_text = "".join(orig_tokens) if is_chinese else " ".join(orig_tokens)
...
def get_final_text(pred_text, orig_text, do_lower_case, verbose_logging=False, is_chinese=True):
    ...
    tok_text = "".join(tokenizer.tokenize(orig_text)) if is_chinese else orig_text
