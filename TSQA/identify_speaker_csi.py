...
para_path = './../dataset/proc_data/CSI/test.json'
java_path="./stanford-corenlp-4.5.4"
out_path="./../analysis/result_records/CSI"
...
def StartServer(port):
    subprocess.run(
        ["java", "-Xmx8g", "-cp", f"{java_path}/*", "edu.stanford.nlp.pipeline.StanfordCoreNLPServer","-outputFormat","json",
         "-serverProperties", f"{java_path}/StanfordCoreNLP-Quote-Chinese.properties", "-port", f"{port}", "-timeout", "150000"])
...
if __name__=='__main__':
    ...
    with open(os.path.join(out_path,'TSQA_CSI_result_records.json'),'w') as f:
